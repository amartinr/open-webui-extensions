...
**Delimiter semantics — live-verified 2026-08-21 (§8.9.1):** `pinned:` and **`tag:`** act as **scope limiters** over the free text (`"Open WebUI"` → 37 chats, `"Open WebUI pinned:true"` → 0, `"manchego tag:comfyui"` → 0); the brief's claim that `tag:` relaxes the text was **verified false** (see §8.9.1). **`folder:` is NOT a usable scope limiter for real folder names at the backend level** (verified live + source, §8.9.7): the prefix splits on spaces and requires an exact normalized name match, so multi-word folder names never filter (the leftover words leak into the text) and the folder id never works. **Fixed client-side in the tool (v0.23.0, task 9.7):** `search_chats` resolves `folder:` names against `GET /api/v1/folders/` (greedy phrase match, underscore rewrite, leak stripping, unknown folder → clean error) — see §8.9.7. A zero-match lone `tag:` query triggers the backend's **orphan-tag cleanup** (documented in §8.9.1 — not a bug; reachable only via `get_chats(tag=)` since 9.8).
...
> **Implemented:** all rows below are live methods in the tool (v0.23.0). Iteration 8 (tags, folders, archived, usage stats) is complete; Iteration 9 tasks 8.9.7–8.9.10 (folder-name resolution, search-term requirement, chat-list unification, `_my_`-drop) landed in v0.21.0–v0.23.0.
...
| `output_format` | `markdown` (admin + per-user, default `markdown`) | Format of the response returned to the model: `markdown` (default, see §8.8) or `json`. Per-user choice from the chat session overrides the admin valve; the admin valve is the default when the user has not chosen |
...
**Decision:** the tool returns the response to the model as **Markdown by default** (plain text, no code fences around whole payloads, no JSON). A **per-user `output_format` valve** (`markdown` default | `json`) lets each user choose JSON when a specific model handles it better — the per-user choice overrides the **admin valve**, which acts as the default when the user has not chosen (see constraints below).
...
- `output_format` is a **per-user valve** (dropdown Markdown/JSON, default Markdown) — the per-user choice **overrides** the **admin valve**, which is the default when the user has not chosen (the tool's built-in default is Markdown).
...
| 8.9.7 | `search_chats` `folder:` prefix — fix folder-NAME search (multi-word names broken; user report 2026-08-21) | ✅ **DONE (v0.23.0)** |
...
10. **Search filter prefixes are scope limiters — including `tag:` (verified live + v0.10.2 source, 2026-08-21).** The brief claimed `tag:` was a standalone filter; **verified false**: `get_chats_by_user_id_and_search_text` strips all prefixes and ANDs the text search with `EXISTS(meta.tags = …)`; multi-tag is AND; `tag:none` is `NOT EXISTS`. Live: `"manchego tag:comfyui"` → 0 (a standalone filter would return the tag's 3 chats). No client-side normalization is needed. **Related intended behavior (documented, not a bug):** a `tag:` query with zero matches triggers the backend's **orphan-tag cleanup** — the catalog entry is deleted (per-chat inline `meta.tags` untouched; a typo'd tag deletes nothing). **After task 9.8 (v0.22.0) this cleanup is only reachable via `get_chats(tag=)`**: a lone `tag:` in `search_chats` is now an error before any request, so `search_chats` can never trigger it. The archived asymmetry is likewise only observable via `get_chats(tag=)` (`search` excludes archived chats, `POST /chats/tags` does not) — documented upstream asymmetry (Iteration 9 task 1, §8.9.1).
11. **The `folder:` search prefix is broken for multi-word folder names (verified live + source, 2026-08-21).** `get_chats_by_user_id_and_search_text` splits the text on spaces, so `folder:Open WebUI meta` queries only `"Open"`; `search_folders_by_names` requires an **exact normalized match** of the full name (`[\s_]+`→space, lowercase) → no match → the folder filter is silently skipped and the leftover words leak into the free text; the folder **id never matches** (names only). The backend's own normalization makes the **underscore-joined single token** work (`folder:open_webui_meta`, `_`≡space — live: 38 chats). `GET /api/v1/chats/folder/{id}` → 401 for the user role on this instance. **FIXED in the tool (Iteration 9 task 9.7, v0.23.0):** `_resolve_folder_filters` resolves folder names client-side (greedy phrase match against `GET /api/v1/folders/` with backend normalization, underscore rewrite, leak stripping, clean error listing valid names for unknown folders) — see §8.9.7.

---

## 10. Tests performed (evidence)

### 8.9.7 Search: `folder:` prefix — folder-name resolution ✅ DONE (v0.23.0)
