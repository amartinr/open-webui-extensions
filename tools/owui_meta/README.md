...
- **`get_file_content` shows the file in the conversation** — images are **embedded inline in the message** (HTML `embeds` mechanism, styled like a snippet — not markdown, not artifacts); text and other binaries appear as an attachment with a clean 100-char snippet / note in the text.
