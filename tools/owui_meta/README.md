...
- **Progress status events** (`verbose` valve, admin + per-user, default on) — the UI shows "Querying your chats…"-style progress while a method runs. **Failures are returned to the model as plain text** (`Error: …`, visible in the tool-call output), **never as a `chat:message:error` event** (v0.25.0: in Open WebUI v0.10.2 that event marks the message errored and blocks the next send, breaking the conversation). A batch delete with failures reports "N of M file(s) could not be deleted" in the returned text.
...
- `search_chats(text)` — **requires a real text term** (a call whose tokens are only UI filter prefixes, e.g. `pinned:true` or `tag:none`, returns a short pointer to the right list tool — `get_chats(scope=…)`, `get_chats(tag=…)`, `get_tags()` or `get_folders` — instead of searching; a valid `folder:` filter is the one exception). Accepts the UI filter prefixes server-side as refinements: `tag:name`, `folder:name`, `pinned:true/false`, `archived:true/false`, `shared:true/false`, `tag:none` — and surfaces the per-result `snippet`. **`folder:` names are resolved client-side** (v0.23.0): multi-word names work with spaces or underscores (`folder:Open WebUI meta` ≡ `folder:open_webui_meta`); an unknown folder name returns a clean error listing the valid names.
